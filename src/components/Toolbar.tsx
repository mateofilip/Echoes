...
const QuoteToolbar = forwardRef<ToolbarRef, QuoteToolbarProps>(
  (
    ...
    const buttonBase =
      "group relative flex h-10 w-10 cursor-pointer items-center justify-center rounded-full text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/20";
    ...
    return (
      <div className="fixed bottom-4 left-1/2 z-40 flex -translate-x-1/2 justify-center">
        <motion.div
          ...
          transition={prefersReducedMotion ? { duration: 0 } : { type: "spring", bounce: 0, duration: 0.45 }}
          className="flex items-center gap-1.5 rounded-full border border-stone-800 bg-stone-900 p-2 shadow-[0_16px_40px_-12px_rgba(0,0,0,0.6),inset_0_1px_0_rgba(255,255,255,0.06)] will-change-transform"
          role="toolbar"
        ...
        >
          ...
          <motion.button
            ...
            whileHover={prefersReducedMotion ? undefined : { backgroundColor: "rgba(255,255,255,0.08)" }}
            whileTap={prefersReducedMotion ? undefined : { scale: 0.93, backgroundColor: "rgba(255,255,255,0.12)" }}
            transition={hoverSpring}
            className={`${buttonBase} bg-transparent hover:bg-transparent`}
            style={{ backgroundColor: "rgba(255,255,255,0)" }}
            aria-label="View saved quotes"
          >
            <motion.span
              animate={!prefersReducedMotion && activeButton === "drawer" ? { scale: 1.15 } : { scale: 1 }}
            ...
            >
              <div className="inline-flex w-auto items-center gap-2 whitespace-nowrap rounded-full border border-stone-800 bg-stone-900 px-3 py-2 text-xs font-medium tracking-tight text-white shadow-xl">
                Saved
          ...
          >
            <motion.span
              animate={!prefersReducedMotion && activeButton === "reload" ? { rotate: 240, scale: 1.15 } : { rotate: 0, scale: 1 }}
          ...
          <motion.button
            ...
            whileHover={prefersReducedMotion || !canGoPrevious ? undefined : { backgroundColor: "rgba(255,255,255,0.08)" }}
            whileTap={prefersReducedMotion || !canGoPrevious ? undefined : { scale: 0.93, backgroundColor: "rgba(255,255,255,0.12)" }}
            transition={hoverSpring}
            className={`${buttonBase} bg-transparent disabled:pointer-events-none disabled:text-white/30`}
            style={{ backgroundColor: "rgba(255,255,255,0)" }}
            disabled={!canGoPrevious}
            aria-label="Previous quote"
          >
            <motion.span animate={!prefersReducedMotion && activeButton === "prev" ? { scale: 1.15 } : { scale: 1 }} transition={tapSpring} className="flex will-change-transform">
          ...
          <motion.button
            ...
            whileHover={prefersReducedMotion || !canGoNext ? undefined : { backgroundColor: "rgba(255,255,255,0.08)" }}
            whileTap={prefersReducedMotion || !canGoNext ? undefined : { scale: 0.93, backgroundColor: "rgba(255,255,255,0.12)" }}
            transition={hoverSpring}
            className={`${buttonBase} bg-transparent disabled:pointer-events-none disabled:text-white/30`}
            style={{ backgroundColor: "rgba(255,255,255,0)" }}
            disabled={!canGoNext}
          ...
          >
            <motion.span animate={!prefersReducedMotion && activeButton === "next" ? { scale: 1.15 } : { scale: 1 }} transition={tapSpring} className="flex will-change-transform">
          ...
          >
            <motion.span animate={!prefersReducedMotion && activeButton === "save" ? { scale: 1.18 } : { scale: 1 }} transition={tapSpring} className="flex will-change-transform">
              {isSaved ? <BookmarkCheck size={20} strokeWidth={1.7} className="fill-white text-white" /> : <Bookmark size={20} strokeWidth={1.7} />}
            </motion.span>
            <motion.div
          ...
          >
            <Info size={20} strokeWidth={1.7} className="will-change-transform" />
            <motion.div
